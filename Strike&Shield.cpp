...
{ 
	...
	int x = 13;
	int y = 7;
	gotoxy(x, y  ); printf(" _____ _         _                          _____ _         _    _ ");
	gotoxy(x, y+1); printf("|   __| |_____ _| |__  ___       ____      |   __| |_ _ ___| |__| |");
	gotoxy(x, y+2); printf("|__   |  _|  _| |    || ._|     |____|     |__   |   | | ._| | .  |");
	gotoxy(x, y+3); printf("|_____|_| |_| |_|_|\\_\\|___|                |_____|_|_|_|___|_|____|");

	gotoxy(x, y+5); printf("                             Covid Shooter");
	gotoxy(x, y+6); printf("                         Press any key to play");
	gotoxy(x, y+7); printf("                     developed by SRY TEAM. - 2021");
}
	...
	public:
		...
		bool isOut()
		{
			...
			{ 
				...
				return true; // And informs the same that it should no longer exist :c
			}
			else
			{
				return false;
		...
		for(asteroid = Asteroids.begin(); asteroid != Asteroids.end(); asteroid++)
		{
			...
			{ 
				...
				{ 
					...
					gotoxy(astX,astY); printf("X"); printf(" "); // it tries to signal a collision, sometimes the X remains theme...
